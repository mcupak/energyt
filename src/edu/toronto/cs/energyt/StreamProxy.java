	...
	protected Void doInBackground(Void... unused) {
		...
		while (isRunning) {
			...
			try {
				...
				mainApp.clientSocketThread = new StreamToMediaPlayerTask(
						client, 0);
				mainApp.clientSocketThread.executeOnExecutor(
						AsyncTask.THREAD_POOL_EXECUTOR, (Void[]) null);
			} catch (SocketTimeoutException e) {
				// Do nothing
			} catch (IOException e) {
				Log.e(MainActivity.TAG,
						"StreamProxy: Error connecting to client", e);
			}
		}
		Log.d(MainActivity.TAG,
				"StreamProxy: Proxy interrupted. Shutting down.");
				...
				while (isRunning && cbToSend > 0 && !client.isClosed()) {
					...
					if (file.exists()) {
						...
						while (cbToSendThisBatch > 0) {
							// if (isCancelled()) {
							// Log.d(MainActivity.TAG,
							// "Closing socket connection - thread cancelled.");
							// break;
							// }
							// how much to send for this exact next packet to
							// mediaplayer (client)
							...
							if (cbRead == -1) {
								...
								mainApp.setStreamedSize(mainApp
										.getStreamedSize() + cbRead);
							}
						...
						// if (isCancelled()) {
						// Log.d(MainActivity.TAG,
						// "Closing socket connection - tread cancelled.");
						// break;
						// }
					}
					...
					if (cbSentThisBatch == 0) {
						Log.d(MainActivity.TAG,
								"Blocking until more data appears");
						Thread.sleep(1000);
					}
				}
